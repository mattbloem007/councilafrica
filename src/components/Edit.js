...
class Edit extends Component {
  ...
  render() {
    ...
    return (
        <Query query={GET_MEMBER} variables={{ memberId: this.props.match.params.id }}>
            {({ loading, error, data }) => {
                if (loading) return 'Loading...';
                if (error) return `Error! ${error.message}`;
        
                return (
                    <Mutation mutation={UPDATE_MEMBER} key={data.member._id} onCompleted={() => this.props.history.push(`/dashboard`)}>
                    ...
                    <Form onSubmit={e => {
                       ...
                       updateMember({ variables: { id: data.member._id, firstname: this.state.firstname, lastname: this.state.lastname, dateofbirth: this.state.dateofbirth , email: this.state.email, cellno: this.state.cellno, race: this.state.race, gender: this.state.gender, country: this.state.country, province: this.state.province, city: this.state.city, occupation: this.state.occupation, industry: this.state.industry, education: this.state.education, income: this.state.income, alc_brands: this.state.alc_brands, tob_brands: this.state.tob_brands, intrests: this.state.intrestFinal, myshopping: this.state.shoppingFinal, recruit: this.state.recruit } });
                        ...
                        <FormGroup row>
                          ...
                          <Col xs="12" md="9">
                              <Input defaultValue={data.member.firstname} type="text" id="firstname" placeholder="Enter your first name" onChange={(e) => {this.setState({firstname: e.target.value})}}/>
                          </Col>
                       </FormGroup>
                       <FormGroup row>
                          <Col md="3">
                          ...
                          <Col xs="12" md="9">
                              <Input defaultValue={data.member.lastname} type="text" id="lastname" placeholder="Enter your last name" onChange={(e)=>{this.setState({lastname: e.target.value})}}/>
                          </Col>
                        </FormGroup>
                      <FormGroup row>
                        <Col md="3">
                          ...
                          <option value="0">Please select</option>
                            {
                                provinces.map(province => {
                                  if (this.state.country == "South Africa") {
                                    ...
                                    return (<option value="N/A">No Province</option>)
                                  }
                                })
                            }
                          </Input>
